...
class DCGAN():
    def __init__(self, lr, beta1, nz, batch_size, num_showimage, device, model_save_path, figure_save_path, generator, discriminator, dataloader):
        self.real_label = 1
        self.fake_label = 0
        self.nz = nz
        ...
        self.opt_G = torch.optim.Adam(
            self.G.parameters(), lr=lr, betas=(beta1, 0.999))
        self.opt_D = torch.optim.Adam(
            self.D.parameters(), lr=lr, betas=(beta1, 0.999))
        self.criterion = nn.BCELoss().to(device)

        self.dataloader = dataloader
        self.fixed_noise = torch.randn(
            self.num_showimage, nz, 1, 1, device=device)
    ...
    def train(self, num_epochs):
        ...
        for epoch in range(num_epochs):
            ...
            for i, data in enumerate(self.dataloader):
                ...
                b_size = x.size(0)  # batch_size
                lbx = torch.full((b_size,), self.real_label,
                ...
                gz = self.G(z)
                lbz1 = torch.full((b_size,), self.fake_label,
                                  dtype=torch.float, device=self.device)
                ...
                lbz2 = torch.full((b_size,), self.real_label,
                                  dtype=torch.float, device=self.device)
            ...
            if (epoch+1) % 10:
                torch.save(self.D.state_dict(), self.model_save_path +
                        'disc_{}.pth'.format(epoch))
                torch.save(self.G.state_dict(), self.model_save_path +
                        'gen_{}.pth'.format(epoch))
        ...
        plt.legend()
        plt.savefig(self.figure_save_path + str(num_epochs) +
                    'epochs_' + 'loss.jpg', bbox_inches='tight')
        ...
        plt.legend()
        plt.savefig(self.figure_save_path + str(num_epochs) +
                    'epochs_' + 'D(x)D(G(z)).jpg', bbox_inches='tight')

        fig = plt.figure(3, figsize=(5, 5))
        plt.axis("off")
        ims = [[plt.imshow(item.permute(1, 2, 0), animated=True)]
               for item in self.img_list]
        ani = animation.ArtistAnimation(
            fig, ims, interval=1000, repeat_delay=1000, blit=True)
        HTML(ani.to_jshtml())
        # ani.to_html5_video()
        ani.save(self.figure_save_path + str(num_epochs) +
                 'epochs_' + 'generation.gif')
        ...
        plt.imshow(utils.make_grid(
            real[0][:self.num_showimage] * 0.5 + 0.5, nrow=10).permute(1, 2, 0))

        # Load the Best Generative Model
        # self.G.load_state_dict(
        #     torch.load(self.model_save_path + 'gen_{}.pth'.format(epoch), map_location=torch.device(self.device)))
        self.G.eval()
        ...
        fake = utils.make_grid(
            fake[:self.num_showimage] * 0.5 + 0.5, nrow=10).permute(1, 2, 0)
        plt.imshow(fake)

        # Save the comparation result
        plt.savefig(self.figure_save_path + str(num_epochs) +
                    'epochs_' + 'result.jpg', bbox_inches='tight')
        # plt.show()
    ...
    def test(self, epoch):
        ...
        plt.title("Real Images")
        real = next(iter(self.dataloader))  # real[0]image,real[1]label
        real = utils.make_grid(
            real[0][:self.num_showimage] * 0.5 + 0.5, nrow=10)
        plt.imshow(real.permute(1, 2, 0))

        # Load the Best Generative Model
        self.G.load_state_dict(torch.load(
            self.model_save_path + 'gen_{}.pth'.format(epoch), map_location=torch.device(self.device)))
        self.G.eval()
        # Generate the Fake Images
        with torch.no_grad():
